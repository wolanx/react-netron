export class Decoder {
    ...
    static open(data, encoding) {
        if (typeof data === 'string') {
            return new Decoder.String(data)
        }
        const assert = (encoding, condition) => {
            if (encoding && encoding !== condition) {
                throw new MyError('Invalid encoding \'' + encoding + '\'.')
            }
        }
        ...
        if (length >= 4 && buffer[0] === 0x84 && buffer[1] === 0x31 && buffer[2] === 0x95 && buffer[3] === 0x33) {
            throw new MyError('Unsupported GB-18030 encoding.')
        }
        if (length > 4 && (length % 2) == 0 && (buffer[0] === 0x00 || buffer[1] === 0x00 || buffer[2] === 0x00 || buffer[3] === 0x00)) {
            ...
            if (lo[0x00] === 0 && (hi[0x00] / (length >> 1)) > 0.5) {
                assert(encoding, 'utf-16')
                return new Decoder.Utf16LE(buffer, 0)
            }
            if (hi[0x00] === 0 && (lo[0x00] / (length >> 1)) > 0.5) {
                assert(encoding, 'utf-16')
                return new Decoder.Utf16BE(buffer, 0)
            }
        }
        if (encoding && (encoding.startsWith('iso-8859-') || encoding.startsWith('latin-'))) {
            return new Decoder.Latin1(buffer, 0)
        }
        assert(encoding, 'utf-8')
        return new Decoder.Utf8(buffer, 0, encoding === 'utf-8')
    }
}

Decoder.String = class {
...
Decoder.Utf8 = class {
    ...
    decode() {
        ...
        if (this.fatal) {
            throw new MyError('Invalid utf-8 character.')
        }
        return String.fromCharCode(0xfffd)
    }
}

Decoder.Latin1 = class {
...
export const TextReader = Reader
