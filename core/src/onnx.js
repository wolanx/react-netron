...
import {TextReader, Decoder} from './text'
import onnxMetadata from './onnx-metadata.json'
...
export class ModelFactory {
    ...
    async open(context, target) {
        ...
        console.log("target", target, context.identifier)
        switch (target) {
            case 'onnx.pbtxt.ModelProto':
                // ./onnx-proto
                try {
                    onnx.proto = OnnxProto
                    const stream = context.stream
...
onnx.Metadata = class {
    ...
    static async open(context) {
        ...
        try {
            console.log('onnxMetadata')
            onnx.Metadata._metadata = new onnx.Metadata(onnxMetadata)
            return onnx.Metadata._metadata
        } catch (error) {
...
onnx.Reader.text = class {
    ...
    read() {
        const decoder = Decoder.open(this._stream)
