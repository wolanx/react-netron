...
zip.Inflater = class {
    ...
    inflateRaw(data, length) {
        ...
        if (typeof process === 'object' && typeof process.versions == 'object' && typeof process.versions.node !== 'undefined') {
            // todo error
            // buffer = zlib.inflateRawSync(data)
        } else {
    ...
    _decodeTrees(reader) {
        ...
        for (let i = 0; i < hlit + hdist;) {
            ...
            switch (literal) {
                case 16:
                    length = reader.bits(2) + 3
                    break
                case 17:
                    length = reader.bits(3) + 3
                    value = 0
                    break
                case 18:
                    length = reader.bits(7) + 11
                    value = 0
                    break
                default:
                    length = 1
...
export const Inflater = zip.Inflater
