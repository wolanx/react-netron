...
protobuf.BinaryReader = class {
    ...
    uint32() {
        ...
        if (this.byte() !== 255 || this.byte() !== 255 || this.byte() !== 255 || this.byte() !== 255 || this.byte() !== 1) {
            throw new ProtobufError('Varint is not 32-bit.')
        }
        return value
    }
    ...
    floats(obj, tag) {
        if ((tag & 7) === 2) {
            if (obj && obj.length > 0) {
                throw new ProtobufError('Invalid packed float array.')
            }
    ...
    skipType(wireType) {
        switch (wireType) {
            ...
            case 5:
                ...
                break
            default:
                throw new ProtobufError('Invalid type ' + wireType + ' at offset ' + this._position + '.')
        }
    }
    ...
    _varint() {
        ...
        throw new ProtobufError('Invalid varint encoding.')
    }
...
protobuf.TextReader = class {
    ...
    static open(data) {
        if (data) {
            const buffer = data instanceof Uint8Array ? data : data.peek()
            const decoder = Decoder.open(buffer)
            let first = true
    ...
    constructor(buffer) {
        this._decoder = Decoder.open(buffer)
        this.reset()
    }
    ...
    start() {
        ...
        if (this._depth <= 0) {
            throw new ProtobufError('Invalid depth ' + this.location())
        }
        if (this._token === '}') {
            this.expect('}')
            this.match(';')
        ...
        if (this._token === undefined) {
            if (this._depth !== 1) {
                throw new ProtobufError('Unexpected end of input' + this.location())
            }
            this._depth--
            return true
        }
    ...
    integer() {
        ...
        if (Number.isNaN(token - value)) {
            throw new ProtobufError('Couldn\'t parse integer \'' + token + '\'' + this.location())
        }
        this.next()
        this.semicolon()
        return value
    ...
    double() {
        ...
        switch (token) {
            ...
            default:
                ...
                if (Number.isNaN(token - value)) {
                    throw new ProtobufError('Couldn\'t parse float \'' + token + '\'' + this.location())
                }
                break
        }
        this.next()
    ...
    string() {
        const token = this._token
        if (token.length < 2) {
            throw new ProtobufError('String is too short' + this.location())
        }
        const quote = token[0]
        if (quote !== '\'' && quote !== '"') {
            throw new ProtobufError('String is not in quotes' + this.location())
        }
        if (quote !== token[token.length - 1]) {
            throw new ProtobufError('String quotes do not match' + this.location())
        }
    ...
    bool() {
        ...
        switch (token) {
            ...
            case '0':
                ...
                return false
            default:
                throw new ProtobufError('Couldn\'t parse boolean \'' + token + '\'' + this.location())
        }
    }
    ...
    enum(type) {
        ...
        if (Object.prototype.hasOwnProperty.call(type, token)) {
            ...
            if (Number.isNaN(token - value)) {
                throw new ProtobufError('Couldn\'t parse enum \'' + (token === undefined ? '' : token) + '\'' + this.location())
            }
        }
        this.next()
        this.semicolon()
        ...
        while (!this.end()) {
            ...
            switch (tag) {
                ...
                case 'value':
                    ...
                    break
                default:
                    throw new ProtobufError('Unsupported entry tag \'' + tag + '\'.')
            }
        }
        obj[k] = v
    }
    ...
    handle(token) {
        throw new ProtobufError('Unexpected token \'' + token + '\'' + this.location())
    }

    field(token /*, module */) {
        throw new ProtobufError('Unsupported field \'' + token + '\'' + this.location())
    }
        ...
        switch (c) {
            ...
            case '\'': {
                ...
                for (; ;) {
                    c = this._decoder.decode()
                    if (c === undefined || c === '\n') {
                        throw new ProtobufError('Unexpected end of string' + this.location())
                    }
                    if (c == '\\') {
                        c = this._decoder.decode()
                        if (c === undefined || c === '\n') {
                        ...
                        switch (c) {
                            ...
                            case 'X': {
                                ...
                                for (let xi = 0; xi < 2; xi++) {
                                    ...
                                    if (xd === -1) {
                                        throw new ProtobufError('Unexpected hex digit \'' + xd + '\' in bytes string' + this.location())
                                    }
                                    value = value << 4 | xd
                                }
                            ...
                            default: {
                                if (c < '0' || c > '9') {
                                    throw new ProtobufError('Unexpected character \'' + c + '\' in string' + this.location())
                                }
                                let value = 0
                                let od = c
                                if (od < '0' || od > '9') {
                                    throw new ProtobufError('Unexpected octal digit \'' + od + '\' in bytes string' + this.location())
                                }
                                od = od.charCodeAt(0)
                                value = value << 3 | od - 48
            ...
            default: {
                throw new ProtobufError('Unexpected token \'' + c + '\'' + this.location())
            }
        }
    }
    ...
    expect(value) {
        if (this._token !== value) {
            throw new ProtobufError('Unexpected \'' + this._token + '\' instead of \'' + value + '\'' + this.location())
        }
        this.next()
    }
...
// export const Error = ProtobufError
