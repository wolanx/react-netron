...
export class BrowserHost {
    ...
    async _openModel(url, identifier) {
        ...
        try {
            ...
            context = new Context(this, url, identifier, stream)
    ...
    async _open(file, files) {
        this._view.show('welcome spinner')
        const context = new BrowserFileContext(this, file, files)
        try {
            await context.open()
            this._telemetry.set('session_engaged', 1)
            await this._view.open(context)
    ...
    _message(message, action) {
        // todo del
        console.warn('message del', message, action)
    }
}
                ...
                if (encoding) {
                    ...
                    if (position === 0 && buffer.length === blob.size) {
                        ...
                        if (position < blob.size) {
                            ...
                            const stream = new FileStream(chunks, size, 0, position)
                            resolve(stream)
                        }
                    }
                }
...
export class FileStream {
    ...
    stream(length) {
        const file = new FileStream(this._chunks, this._size, this._position, length)
        this.skip(length)
        return file
    }
